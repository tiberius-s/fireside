//! Design tokens — semantic color system for Slideways.
//!
...
/// Complete set of semantic design tokens for a Slideways theme.
///
...
impl DesignTokens {
    /// Convert tokens back to the legacy `Theme` struct for backward compat.
    #[must_use]
    pub fn to_legacy_theme(&self) -> crate::model::theme::Theme {
        crate::model::theme::Theme {
    ...
    /// Create tokens from a legacy `Theme`.
    #[must_use]
    pub fn from_legacy_theme(theme: &crate::model::theme::Theme) -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn legacy_theme_roundtrip() {
        let tokens = DesignTokens::default();
        let legacy = tokens.to_legacy_theme();
        let back = DesignTokens::from_legacy_theme(&legacy);
