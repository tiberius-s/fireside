//! Slide layout templates — reusable layouts with Ratatui area mappings.
//!
...
mod tests {
    ...
    fn two_column_has_secondary() {
        let areas = SlideTemplate::TwoColumn.compute_areas(test_area(), Breakpoint::Standard);
        assert!(areas.secondary.is_some());
    }

    #[test]
    fn speaker_notes_has_secondary() {
        let areas = SlideTemplate::SpeakerNotes.compute_areas(test_area(), Breakpoint::Standard);
        assert!(areas.secondary.is_some());
    }

    #[test]
    fn title_no_secondary() {
        let areas = SlideTemplate::Title.compute_areas(test_area(), Breakpoint::Standard);
    ...
    fn compact_breakpoint_reduces_padding() {
        ...
        let compact = SlideTemplate::BulletList.compute_areas(area, Breakpoint::Compact);
        let wide =
            SlideTemplate::BulletList.compute_areas(Rect::new(0, 0, 160, 50), Breakpoint::Wide);
    ...
    fn parse_template_names() {
        assert_eq!(
            SlideTemplate::from_name("title"),
            Some(SlideTemplate::Title)
        );
        assert_eq!(
            SlideTemplate::from_name("two-column"),
            Some(SlideTemplate::TwoColumn)
        );
        assert_eq!(
            SlideTemplate::from_name("code_focus"),
            Some(SlideTemplate::CodeFocus)
        );
        assert_eq!(
            SlideTemplate::from_name("speaker-notes"),
            Some(SlideTemplate::SpeakerNotes)
        );
        assert_eq!(SlideTemplate::from_name("unknown"), None);
    }
