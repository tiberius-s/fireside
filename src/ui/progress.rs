//! Progress bar widget showing slide position and elapsed time.
...
pub fn render_progress_bar(
    ...
    let slide_info = format!(" Slide {} / {} ", current + 1, total);
    let time_info = format!(" {minutes:02}:{seconds:02} ");

    // Calculate padding to right-align the time
    let padding_len = (area.width as usize).saturating_sub(slide_info.len() + time_info.len());
    let padding = " ".repeat(padding_len);

    let line = Line::from(vec![
        Span::styled(slide_info, bold),
